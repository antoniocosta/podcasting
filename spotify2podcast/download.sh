...
function download_playlist {
	...
	echo 'Choose how to download:'
	local options=("SpotDL (YouTube)" "ZSpotify (Spotify)" "Skip. Don't download." "Quit")
	...
	do
		case $opt in
			${options[0]})
				...
				# Jul 18 '23: pipx stopped working. using installed version isntead.
				# spotdl download $SPOTIFY_PLAYLIST_URL --m3u {list[0]} --save-file "./_save_file.spotdl" --preload --print-errors --bitrate 256k
			...
			${options[1]})
				echo "You chose $REPLY: $opt"
				echo "Downloading all playlist's songs with ZSpotify..."

				# Lets use ZSpotify and pass all default options via command line
				# Use only absolute paths or else it's too complicated

				BITRATE=160 # ZSpotify generates mp3s at 160 kbp/s bitrate. Let's set it.

				local ep_subdir=$(pwd) # remember current episode subdir so we go back to it
				local tmp_subdir='/home/abmc/podcasting/docs/all-my-favorite-songs/downloads/_tmp_zspotify/'
				cd '/home/abmc/_3rd_party/zspotify' # where is zspotify? cd there

				python3.9 zspotify \
				--root-path $tmp_subdir \
				--root-podcast-path $tmp_subdir \
				--skip-existing-files true \
				--skip-previously-downloaded false \
				--download-format 'mp3' \
				--force-premium false \
				--anti-ban-wait-time 1 \
				--override-auto-wait false \
				--chunk-size 50000 \
				--split-album-discs false \
				--download-real-time false \
				--language 'en' \
				--credentials-location 'credentials.json' \
				$SPOTIFY_PLAYLIST_URL

				echo "ZSpotify still doesn't save m3u :( Let's generate it..."
				cd $tmp_subdir
				cd */ # There should be only one subdir (w playlist name) inside
				local playlist_name=${PWD##*/} # Assign current directory name (without full path) to var
				ls -trN *.mp3 > "$playlist_name.m3u" # Create playlist. Important: Relies on file dates for ordering so it only works if all files were downloaded sequentially!

				echo 'Moving files and cleaning up...'
				mv * $ep_subdir # move all files to episode subdir
				cd $ep_subdir # cd back to episode subdir
				# Cleanup:
				rm -f "$tmp_subdir$playlist_name/.song_ids"
				rmdir "$tmp_subdir$playlist_name"
				rmdir "$tmp_subdir"
